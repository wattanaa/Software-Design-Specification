                  ...
                  <div className="col-sm-8">
                    {selectedEvent.description ? (
                      selectedEvent.description.split('\n').map((line, index) => {
                        // ✅ ตรวจสอบและทำให้ "📌 รายละเอียดการจอง" เป็นตัวหนา
                        if (line.includes("📌 **รายละเอียดการจอง**")) {
                          return (
                            <b key={index} style={{ display: "block" }}>
                              {line}
                            </b>
                          );
                        }

                        // ✅ รายการหัวข้อที่ต้องเป็นตัวหนา
                        const boldFields = [
                          "ชื่อผู้จอง", "เบอร์โทรศัพท์", "Email", "องค์กร", "องค์กร/สำนักงาน",
                          "วัตถุประสงค์", "วัตถุประสงค์ของการใช้", "สนามแข่งขัน", "สนามที่เลือก",
                          "สิ่งอำนวยความสะดวกที่เลือก", "ระบบที่ต้องใช้", "ห้องที่ใช้", "การทำความสะอาด"
                        ];

                        // ✅ ตรวจสอบว่าบรรทัดนั้นเป็นหัวข้อที่ต้องทำตัวหนาหรือไม่
                        for (let field of boldFields) {
                          if (line.includes(field + ":")) {
                            const [label, value] = line.split(":"); // แยกหัวข้อกับค่าข้อมูล
                            return (
                              <div key={index}>
                                <b>{label.trim()}:</b> {value ? value.trim() : ""}
                              </div>
                            );
                          }
                        }

                        // ✅ เปลี่ยนสีของ **สถานะ** ตามสถานะที่กำหนด
                        if (line.includes("✅ **สถานะ:")) {
                          let statusColor = "#000"; // สีเริ่มต้น (สีดำ)
                          let formattedText = line.replace("✅ **สถานะ:", "").replace("**", "").trim(); // ลบ ✅ และ ** ออก

                          if (formattedText === "รออนุมัติ") statusColor = "#FFD700"; // 🟡 สีเหลือง
                          if (formattedText === "รอชำระเงิน") statusColor = "#FFA500"; // 🟠 สีส้ม
                          if (formattedText === "อนุมัติ") statusColor = "#00FF00"; // 🟢 สีเขียว
                          if (formattedText === "ปฏิเสธ") statusColor = "#FF0000"; // 🔴 สีแดง

                          return (
                            <b key={index} style={{ color: statusColor }}>
                              ✅ **สถานะ: {formattedText}**
                            </b>
                          );
                        }
                        ...
                        // ✅ รายการ (`- ข้อมูล`) จะไม่เป็นตัวหนา
                        if (line.startsWith("-")) {
                          return <div key={index} style={{ marginLeft: "15px" }}>{line}</div>;
                        }

                        // ✅ สถานะ ให้แสดงเป็นตัวหนาสีเขียว
                        if (line.includes("✅ **สถานะ")) {
                          return <b key={index} style={{ color: "green" }}>{line}</b>;
                        }

                        // ✅ ถ้าไม่ใช่หัวข้อ ให้แสดงตามปกติ
